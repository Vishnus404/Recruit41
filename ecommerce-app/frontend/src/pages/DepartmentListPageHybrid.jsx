...
const DepartmentListPage = () => {
      ...
      if (useRealData) {
        try {
          ...
          if (response.ok) {
            ...
            if (data.success) {
              console.log('Real API data received:', data.departments);
  ...
  const basicStyles = {
    ...
    productCount: {
      ...
      background: useRealData ? '#48bb78' : '#edf2f7',
      color: useRealData ? 'white' : '#4a5568',
      padding: '0.5rem 1rem',
      borderRadius: '20px',
      fontSize: '0.9rem',
      fontWeight: '500',
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
        <div style={basicStyles.container}>
          <div style={basicStyles.loading}>
            ...
            <p>Loading departments{useRealData ? ' from database' : ''}...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
      <div style={basicStyles.container}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <button 
            style={basicStyles.toggleButton}
            onClick={() => {
              setUseRealData(!useRealData);
              fetchDepartments();
            }}
          >
            {useRealData ? '🔗 Using Real API' : '📱 Using Mock Data'} - Click to {useRealData ? 'Use Mock' : 'Try Real API'}
          </button>
        </div>
              ...
              <div>
                ...
                <div style={basicStyles.description}>
                  ...
                  <small style={{ color: '#a0aec0' }}>
                    {useRealData ? '✅ Real database data' : '📱 Sample data'}
                  </small>
                </div>
        ...
        {departments.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '3rem', background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)' }}>
            ...
            <p style={{ color: '#718096' }}>Please check back later or try toggling the data source.</p>
          </div>
        )}
      </div>
