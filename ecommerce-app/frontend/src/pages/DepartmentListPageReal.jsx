...
const DepartmentListPage = () => {
  ...
  const fetchDepartments = async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('Fetching real departments from API...');
      
      // Try to fetch from real API
      const departmentData = await departmentService.getAllDepartments();
      console.log('Real department data received:', departmentData);
      ...
      setError(`API unavailable: ${err.message}. Showing sample data. Please start the backend server on http://localhost:3000`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={basicStyles.container}>
        ...
        {departments.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '3rem', background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)' }}>
            ...
            <p style={{ color: '#718096' }}>Please check back later or ensure the backend server is running.</p>
          </div>
        )}
      </div>
